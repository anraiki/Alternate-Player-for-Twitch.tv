...
function получитьВерсиюБраузера() {
	if (!получитьВерсиюБраузера._чРезультат) {
		const совпадение = /Firefox\/(\d+)/.exec(navigator.userAgent) || /(?:Chrome|Chromium|Edg)\/(\d+)/.exec(navigator.userAgent);
		получитьВерсиюБраузера._чРезультат = совпадение ? Number(совпадение[1]) : 999;
	}
...
const м_Настройки = (() => {
	...
	const _мноПостоянныеНастройки = new Set([ 'чВерсияНастроек', 'чСлучайноеЧисло', 'сПредыдущаяВерсия', 'чПроверкаВерсии', 'лАвтоперенаправлениеЗамечено' ]);
	...
	const _оНастройки = {
		...
		чПроверкаВерсии: Настройка.Создать(0),
