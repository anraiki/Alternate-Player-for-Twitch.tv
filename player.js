...
const ЗАГРУЖАТЬ_СПИСОК_ВАРИАНТОВ_НЕ_ДОЛЬШЕ = 3e4;
...
const м_Статистика = (() => {
	...
	function ПолученПреобразованныйСегмент(оСегмент) {
		...
		if (оДанные.hasOwnProperty('мбМедиасегмент')) {
			if (оСегмент.лРазрыв && оДанные.fmp4) {
				Узел('статистика-сжатиевидео').textContent = оДанные.лЕстьВидео ? оДанные.сКодеки : '—';
				Узел('статистика-разрешениевидео').textContent = оДанные.лЕстьВидео ? `${оДанные.чШиринаКартинки}x${оДанные.чВысотаКартинки}` : '—';
				Узел('статистика-сжатиезвука').textContent = оДанные.лЕстьЗвук ? 'AAC' : '—';
				Узел('статистика-частотакадров').textContent = '';
				Узел('статистика-битрейтзвука').textContent = '';
			} else if (оСегмент.лРазрыв) {
...
const м_Окно = (() => {
	...
	function закрытьОкно(сИдОкна, лСАнимацией = true) {
		...
		м_События.ПослатьСобытие(`окно-закрыто-${сИдОкна}`);
	}
...
const м_Тащилка = (() => {
	...
	const ОбработатьPointerDown = создатьОбработчикСобытийЭлемента(оСобытие => {
		...
		const идТащится = узНажат.getAttribute('data-тащилка');
		_чИдУказателя = оСобытие.pointerId;
		_оПараметры = new Параметры(узНажат, Узел(идТащится));
		_чВремяПоследнегоПеретаскивания = 0;
		_чНачальнаяX = _чПоследняяX = оСобытие.clientX;
		_чНачальнаяY = _чПоследняяY = оСобытие.clientY;
		м_Журнал.Окак(`[Тащилка] Начинаю перетаскивать ${идТащится} X=${_чНачальнаяX} Y=${_чНачальнаяY} id=${_чИдУказателя} type=${оСобытие.pointerType} primary=${оСобытие.isPrimary}`);
		...
		const перехват = ПолучитьЭлементПерехватчик();
		перехват.setAttribute('data-тащилка-событиявводадля', идТащится);
		перехват.style.setProperty('cursor', getComputedStyle(узНажат).cursor, 'important');
		перехват.classList.add('тащилка-перехват');
		_оПараметры.узТащится.classList.add('тащилка');
		м_События.ПослатьСобытие(`тащилка-перетаскивание-${идТащится}`, _оПараметры);
	});
	...
	function ЗавершитьПеретаскивание(сПричина) {
		if (_оПараметры.чШаг !== 3) {
			...
			м_События.ПослатьСобытие(`тащилка-перетаскивание-${_оПараметры.узТащится.id}`, _оПараметры);
			const перехват = ПолучитьЭлементПерехватчик();
			перехват.style.removeProperty('cursor');
			перехват.classList.remove('тащилка-перехват');
	...
	function ПолучитьЭлементПерехватчик() {
		return м_ПолноэкранныйРежим.ПолучитьЭлемент();
	}
	document.addEventListener('pointerdown', ОбработатьPointerDown, ПАССИВНЫЙ_ОБРАБОТЧИК);
	return {
		ОтменитьПеретаскивание,
		ПолучитьЭлементПерехватчик
	};
})();
...
const м_Новости = (() => {
	...
	const _мНовости = [ [ '2026.6.21', 'J1010', 'F1079' ], [ '2026.6.21', 'F1500', 'F1573', 'F1515' ], [ '2026.6.21', 'J1513', 'F1080' ], [ '2025.5.28', 'J1010', 'F1078' ], [ '2024.7.3', 'J1010', 'F1076' ], [ '2024.6.14', 'J1010', 'F1077' ], [ '2024.6.5', 'J1010', 'F1074' ], [ '2024.5.31', 'F1072', 'F1073' ], [ '2021.12.17', 'J1066', 'F1070', 'F1514' ], [ '2021.12.17', 'J1010', 'F1069' ], [ '2021.3.7', 'J1010', 'F1584' ], [ '2020.10.30', 'J1066', 'F1067' ], [ '2020.10.5', 'J1010', 'F1065' ], [ '2019.10.9', 'J1010', 'F1064' ], [ '2019.3.17', 'J1010', 'F1063' ], [ '2018.10.28', 'J1010', 'F1062' ], [ '2018.8.17', 'J1010', 'F1060' ], [ '2018.7.30', 'J1010', 'F1059' ], [ '2018.6.27', 'J1010', 'F1058' ], [ '2018.6.12', 'J1010', 'F1057' ], [ '2018.5.18', 'J1010', 'F1049' ], [ '2018.4.6', 'J1010', 'F1047' ], [ '2018.3.5', 'J1041', 'F1042' ], [ '2018.2.17', 'J1010', 'F1044' ], [ '2018.1.7', 'J1010', 'F1043' ], [ '2017.11.8', 'J1039', 'F1040' ], [ '2017.11.6', 'J1010', 'F1037' ], [ '2017.10.22', 'J1010', 'F1023' ], [ '2017.10.14', 'J1010', 'F1020' ], [ '2017.9.11', 'J1010', 'F1018' ], [ '2017.8.8', 'J1035', 'F1017' ], [ '2017.6.23', 'J1010', 'F1014' ], [ '2017.5.29', 'J1010', 'F1013' ], [ '2017.3.31', 'J1031', 'F1012' ], [ '2017.2.26', 'J1030', 'F1011' ], [ ПОЛНАЯ_СПРАВКА, 'F1500', 'F1503', 'F1502', 'F1509', 'F1573', 'F1515', 'F1584', 'F1574', 'F1504', 'F1507', 'F1516', 'F1511', 'F1514' ], [ ПОЛНАЯ_СПРАВКА, 'J1513', 'F1570', 'F1581', 'F1517', 'F1506', 'F1510' ], [ ПОКАЗАТЬ_ОДИН_РАЗ, 'F1500', 'F1501' ], [ ДЛЯ_ПЛАНШЕТА, 'J1055', 'F1056' ], [ ПОКАЗЫВАТЬ_ВСЕГДА, 'J1003', 'F1000' ] ];
	function задатьАдресДляПеревода(ссылка, переводимыйТекст) {
		Проверить(ЭтоНепустаяСтрока(переводимыйТекст));
		ссылка.href = `https://translate.google.com/?op=translate&sl=en&text=${encodeURIComponent(переводимыйТекст.trim())}`;
	}
	function добавитьСсылкуДляПеревода(добавитьВ, переводимыйТекст) {
		if (Текст('M0010') !== 'ru') {
			const ссылка = добавитьВ.appendChild(document.createElement('a'));
			ссылка.className = 'новость-перевести js-непереведено';
			ссылка.target = '_blank';
			ссылка.rel = 'noopener noreferrer';
			ссылка.title = Текст('J0148');
			задатьАдресДляПеревода(ссылка, переводимыйТекст);
		}
	}
	...
	function ДобавитьНовости(чДобавитьВерсииСтарше, сДобавитьВерсиюСправки) {
		...
		for (let мНовость of _мНовости) {
			...
			if (сВерсия.startsWith('2000')) {
				if (сВерсия === сДобавитьВерсиюСправки || сВерсия === ПОКАЗЫВАТЬ_ВСЕГДА || сВерсия === ДЛЯ_ПЛАНШЕТА && этоМобильноеУстройство()) {
		...
		элДобавитьВ.addEventListener('mousedown', обработатьПереводНовости);
	...
	function ДобавитьНовость(элДобавитьВ, мНовость, чДатаНовости) {
		...
		добавитьСсылкуДляПеревода(узЗаголовок, '...');
	...
	const обработатьПереводНовости = создатьОбработчикСобытийЭлемента(({target}) => {
		if (!target.classList.contains('js-непереведено')) {
			return;
		}
		target.classList.remove('js-непереведено');
		let переводимыйТекст = '';
		for (let блок = target.parentElement; блок && блок.nodeName !== 'HR'; блок = блок.nextElementSibling) {
			if (блок.nodeName === 'OL' || блок.nodeName === 'UL') {
				for (const пункт of блок.children) {
					переводимыйТекст += ` • ${пункт.innerText}\n`;
				}
				переводимыйТекст += '\n';
			} else {
				Проверить('innerText' in блок);
				переводимыйТекст += `${блок.innerText}\n\n`;
			}
		}
		задатьАдресДляПеревода(target, переводимыйТекст);
	});
	...
	function уведомитьОНеобходимостиОбновитьРасширение(результатПроверки) {
		const текст = м_i18n.InsertAdjacentHtmlMessage('расширениеустарело-текст', 'afterbegin', результатПроверки === -1 ? 'F0680' : 'F0668');
		добавитьСсылкуДляПеревода(текст, текст.textContent);
		м_Окно.открыть('расширениеустарело');
		м_События.ДобавитьОбработчик('окно-закрыто-расширениеустарело', () => {
			м_Настройки.Изменить('чПроверкаВерсии', -Date.now());
		});
	}
	function проверитьПоддержкуТекущейВерсииРасширения() {
		const ИНТЕРВАЛ_ПРОВЕРКИ = 1e3 * 60 * 60 * 24 * 6;
		let результатПроверки = м_Настройки.Получить('чПроверкаВерсии');
		if (результатПроверки === -1 || результатПроверки === -2) {
			уведомитьОНеобходимостиОбновитьРасширение(результатПроверки);
			return;
		}
		const сейчас = Date.now();
		if (Math.abs(сейчас - Math.abs(результатПроверки)) < ИНТЕРВАЛ_ПРОВЕРКИ) {
			return;
		}
		Ждать(null, 900).then(() => м_Загрузчик.Загрузить(null, 'GET', `https://coolcmd.github.io/tw5/version.json?${сейчас}`, 9e4, null, null, 'версия расширения', false, 'json')).then(версии => {
			let расширениеУстарело = false, браузерУстарел = false;
			try {
				const {оПоддерживаемаяВерсия: {оЛиса: {сВерсияРасширения, чВерсияБраузера}}} = версии;
				Проверить(ЭтоНепустаяСтрока(сВерсияРасширения) && Number.isSafeInteger(чВерсияБраузера));
				расширениеУстарело = ПеревестиВерсиюВМиллисекунды(ВЕРСИЯ_РАСШИРЕНИЯ) < ПеревестиВерсиюВМиллисекунды(сВерсияРасширения);
				браузерУстарел = navigator.userAgent.includes('Firefox/') && получитьВерсиюБраузера() < чВерсияБраузера;
			} catch (исключение) {
				throw String(исключение);
			}
			if (!расширениеУстарело) {
				результатПроверки = сейчас;
			} else if (результатПроверки >= 0) {
				результатПроверки = -сейчас;
			} else {
				результатПроверки = +браузерУстарел - 2;
				уведомитьОНеобходимостиОбновитьРасширение(результатПроверки);
			}
			м_Настройки.Изменить('чПроверкаВерсии', результатПроверки);
			м_Настройки.СохранитьИзменения();
		}).catch(ДобавитьОбработчикИсключений(причина => {
			if (typeof причина == 'string') {
				м_Журнал.Ой(`[Новости] Не удалось проверить версию расширения. ${причина}`);
				м_Настройки.Изменить('чПроверкаВерсии', результатПроверки < 0 ? -сейчас : сейчас);
			} else {
				throw причина;
			}
		}));
	...
	function Запустить() {
		...
		проверитьПоддержкуТекущейВерсииРасширения();
	}
	return {
		Запустить,
		ОткрытьНовости,
...
const м_Чат = (() => {
 ...
 //! <iframe>
	...
	const обработатьДвойнойЩелчок = создатьОбработчикСобытийЭлемента(событие => {
		if (событие.currentTarget === событие.target && событие.currentTarget.getAttribute('data-тащилка-событиявводадля') === 'размерчата') {
			ОтменитьПеретаскиваниеПанели();
			const положение = ПолучитьПоложениеПанели();
			м_Журнал.Вот(`[Чат] Восстанавливаю размер стороны ${положение}`);
			if (положение === ПРАВАЯ_СТОРОНА || положение === ЛЕВАЯ_СТОРОНА) {
				const {пНачальное} = м_Настройки.ПолучитьПараметрыНастройки('чШиринаПанелиЧата');
				м_Настройки.Изменить('чШиринаПанелиЧата', пНачальное);
				_узЧат.width = пНачальное;
			} else {
				const {пНачальное} = м_Настройки.ПолучитьПараметрыНастройки('чВысотаПанелиЧата');
				м_Настройки.Изменить('чВысотаПанелиЧата', пНачальное);
				_узЧат.height = пНачальное;
			}
		}
	});
	...
	function Восстановить() {
		ПрименитьСостояниеПанели();
		ПрименитьПоложениеПанели();
		м_Тащилка.ПолучитьЭлементПерехватчик().addEventListener('dblclick', обработатьДвойнойЩелчок);
...
const м_Проигрыватель = (() => {
	...
	function requestPlayback() {
		// Chromium rejects pending play requests when pause() or load() interrupts them.
		// Startup deliberately probes playback this way, as do buffer and replay resets.
		return _oMediaElement.play().catch(error => {
			if (г_лРаботаЗавершена) {
				return;
			}
			...
			if (error.name === 'NotAllowedError') {
				м_Журнал.Ой('[Player] Playback was blocked by the browser. Click Play to start.');
				м_Управление.ОстановитьПросмотрТрансляции();
				return;
			}
			try {
				...
				// The existing fatal-error reporter throws undefined after shutting down.
				if (reportError !== undefined || !г_лРаботаЗавершена) {
					console.error('[Player] Failed to report the playback error:', error, reportError);
				}
			}
		});
	}
	...
	function Запустить() {
		...
		if (м_Фокусник.ПолучитьСостояние().лПоказан) {
			ПоказатьСостояние('Ой', 'Autoplay is blocked by the browser');
			_oMediaElement.pause();
			подключитьMediaSourceКMediaElement();
			return false;
		}
		ПоказатьСостояние('Окак', 'Playback is blocked until the tab becomes visible');
		_oMediaElement.pause();
		м_События.ДобавитьОбработчик('фокусник-изменилосьсостояние', function ОбработатьПереходНаВкладку({лПоказан}, сСобытие) {
			if (лПоказан) {
				...
				requestPlayback();
				if (!_oMediaElement.paused) {
					ПоказатьСостояние('Окак', 'Playback is now allowed');
					_oMediaElement.pause();
					подключитьMediaSourceКMediaElement();
				} else {
					ПоказатьСостояние('Ой', 'Autoplay is blocked by the browser');
...
const м_Список = (() => {
		...
		_обновить(оОтменаОбещания, чЧерез) {
			...
			if (оСписокВариантов === null) {
				...
				оОбещание = оОбещание.then(() => м_Twitch.ПолучитьАбсолютныйАдресСпискаВариантов(оОтменаОбещания, false, this._лБезРекламы)).then(сРезультат => {
					...
					return м_Загрузчик.Загрузить(оОтменаОбещания, 'GET', сАбсолютныйАдресСпискаВариантов, ЗАГРУЖАТЬ_СПИСОК_ВАРИАНТОВ_НЕ_ДОЛЬШЕ, null, null, `список вариантов ${+this._лБезРекламы}`, false, 'text');
				}).then(сРезультат => {
			...
			оОбещание.then(() => {
				...
				return м_Загрузчик.Загрузить(оОтменаОбещания, 'GET', оВыбранныйВариант.сАбсолютныйАдресСпискаСегментов, ЗАГРУЖАТЬ_СПИСОК_СЕГМЕНТОВ_НЕ_ДОЛЬШЕ, null, null, `список сегментов ${+this._лБезРекламы}`, false, 'text');
			}).then(сРезультат => {
	...
	function РазобратьСписок(лЭтоСписокВариантов, сАбсолютныйАдресСписка, сРазбираемыйСписок) {
		...
		let чВерсия = 1;
		let mapRenditionGroups, моВарианты, оНовыйВариант, сИдТрансляции;
		...
		let streamPosition = NaN, programTime = NaN;
				...
				switch (сНазваниеТега) {
				  case '':
					if (лЭтоСписокВариантов) {
						...
						streamPosition += оНовыйСегмент.чДлительность;
						programTime += оНовыйСегмент.чДлительность * 1e3;
						лРазрыв = false;
				  ...
				  case 'INF':
					{
						...
						оНовыйСегмент.initialization = initialization;
						оНовыйСегмент.streamPosition = streamPosition;
						оНовыйСегмент.programTime = programTime;
				  ...
				  case '-X-PROGRAM-DATE-TIME':
					Проверить(!лЭтоСписокВариантов);
					programTime = Date.parse(сЗначениеТега);
					break;

				  case '-X-KEY':
					Проверить(!лЭтоСписокВариантов);
					if (РазобратьСписокАтрибутов(сЗначениеТега).get('METHOD') !== 'NONE') {
				  ...
				  case '-X-MAP': {
					Проверить(!лЭтоСписокВариантов);
					const attributes = РазобратьСписокАтрибутов(сЗначениеТега);
					браковать(attributes.get('URI') && !attributes.has('BYTERANGE'));
					initialization = ResolveRelativeUrl(attributes.get('URI'), сАбсолютныйАдресСписка);
					break;
				  }
				  ...
				  case '-X-TWITCH-ELAPSED-SECS':
					streamPosition = РазобратьПоложительноеЧисло(сЗначениеТега);
					break;
				  ...
				  case '-X-STREAM-INF':
					{
						...
						const амАтрибуты = РазобратьСписокАтрибутов(сЗначениеТега);
						оНовыйВариант.codecs = амАтрибуты.get('CODECS') || '';
	...
	function ДобавитьСегментыВОчередь(оНовыеВарианты, оНовыеСегменты, оВыбранныйВариант) {
		...
		function добавитьСегментВОчередь(оСегмент, чПорядковыйНомер) {
			...
			if (оСегмент.initialization) {
				оДобавлено.fmp4 = {url: оСегмент.initialization, codecs: оВыбранныйВариант.codecs, streamPosition: оСегмент.streamPosition, programTime: оСегмент.programTime};
			}
...
const м_Загрузчик = (() => {
	...
	function Загрузить(оОтменаОбещания, сМетод, сАдрес, чНеДольше, оЗаголовки, пТело, сНазвание, лЖурнал, пТипДанных) {
		...
		Проверить(пТипДанных === 'arraybuffer' || пТипДанных === 'none' || пТипДанных === 'text' || пТипДанных === 'json' || Number.isFinite(пТипДанных));
		if (оОтменаОбещания && оОтменаОбещания.лОтменено) {
			return Promise.reject(ОтменаОбещания.ПРИЧИНА);
		}
	...
	function ПослатьЗапрос(оЗапрос, лПовторно) {
		...
		оЗапрос.responseType = typeof оЗапрос._пТипДанных == 'number' || оЗапрос._пТипДанных === 'arraybuffer' ? 'arraybuffer' : 'text';
	...
	// Keep only completed initialization downloads: each pending request owns its cancellation.
	const initializationCache = new Map();
	async function loadInitialization(segment, cancellation) {
		if (!segment.fmp4) return;
		const url = segment.fmp4.url;
		let data = initializationCache.get(url);
		if (!data) {
			data = await Загрузить(cancellation, 'GET', url, ЗагружатьСегментНеДольше(segment), null, null, 'MP4 initialization', false, 'arraybuffer');
			initializationCache.set(url, data);
			if (initializationCache.size > 4) initializationCache.delete(initializationCache.keys().next().value);
		}
		segment.fmp4.initialization = data;
	}
	function ЗагрузитьСегмент(оСегмент) {
		...
		const cancellation = оСегмент.пДанные;
		loadInitialization(оСегмент, cancellation).then(() => Загрузить(cancellation, 'GET', сАдрес, ЗагружатьСегментНеДольше(оСегмент), null, null, `сегмент ${оСегмент.чНомер}`, м_Статистика.ОкноОткрыто(), оСегмент.чДлительность)).then(буфДанные => {
...
const м_Twitch = (() => {
	const ЗАГРУЖАТЬ_GQL_НЕ_ДОЛЬШЕ = 3e4;
	...
	function создатьУникальныйИдентификатор(кДлина) {
		...
		const сАлфавит = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
		let сРезультат = '';
		while (сРезультат.length !== кДлина) {
			сРезультат += сАлфавит[Math.floor(Math.random() * сАлфавит.length)];
		}
		return сРезультат;
	}
	...
	function отправитьЗапросGql(оОтменаОбещания, сЗапрос, оПеременные, лПосылатьТокенЗрителя, лПосылатьТокенGql, лПовторятьЗапрос, сНазваниеЗагрузки, чЗагружатьНеДольше = ЗАГРУЖАТЬ_GQL_НЕ_ДОЛЬШЕ) {
	...
	function отправитьПросмотрРекламногоБлока(оСписокСегментов) {
		Ждать(null, 3e3).then(() => {
			return отправитьЗапросGql(null, объединитьЗапросыGql([ создатьСобытиеРекламы('video_ad_impression', оСписокСегментов), создатьСобытиеРекламы('video_ad_quartile_complete', оСписокСегментов, 1), создатьСобытиеРекламы('video_ad_quartile_complete', оСписокСегментов, 2), создатьСобытиеРекламы('video_ad_quartile_complete', оСписокСегментов, 3), создатьСобытиеРекламы('video_ad_quartile_complete', оСписокСегментов, 4), создатьСобытиеРекламы('video_ad_pod_complete', оСписокСегментов) ]), null, true, false, false, `${оСписокСегментов.сТипРекламы} ${оСписокСегментов.сТокенРекламы.slice(-10)}`);
		}).then(моРезультаты => {
	...
	function ПолучитьАбсолютныйАдресСпискаВариантов(оОтменаОбещания, лБезHttps, лБезРекламы) {
		...
		if (!лБезРекламы) {
			...
			if (чПротухнетЧерез > 0) {
				...
				return Promise.resolve(`${ПолучитьАбсолютныйАдресСпискаВариантов._сАдрес}&p=${Math.floor(Math.random() * 9999999)}`);
			}
		}
		return отправитьЗапросGql(оОтменаОбещания, `query(
			...
			let сАдрес = `${лБезHttps ? 'http' : 'https'}://usher.ttvnw.net/api/channel/hls/${encodeURIComponent(_сКодКанала)}.m3u8` + '?allow_audio_only=true' + '&allow_source=true' + '&cdm=wv' + '&platform=web' + '&player_backend=mediaplayer' + '&reassignments_supported=true' + '&supported_codecs=h264' + '&transcode_mode=cbr_v1' + `&token=${encodeURIComponent(сТокен)}` + `&sig=${encodeURIComponent(сПодпись)}`;
			if (!лБезРекламы) {
				_playSessionId = создатьУникальныйИдентификатор(32);
				сАдрес += `&play_session_id=${_playSessionId}`;
	...
	const отправитьДанныеСлеженияЗаПросмотром = ДобавитьОбработчикИсключений(() => {
		Проверить(_сИдТрансляции && _сИдКанала && _playSessionId && _сИдЗрителя);
		отправитьЗапросGql(null, `mutation($input: SendSpadeEventsInput!) {
			...
			input: {
				...
				data: JSON.stringify([ {
					...
					properties: {
						...
						live: true,
						player: 'site',
						play_session_id: _playSessionId,
						user_id: _сИдЗрителя
					}
				} ])
			}
		}, false, false, true, 'слежение за просмотром', ИНТЕРВАЛ_СЛЕЖЕНИЯ_ЗА_ПРОСМОТРОМ / 2).then(оРезультат => {
			if (цепочка(оРезультат.data, 'sendSpadeEvents', 'statusCode') !== 204) {
				throw 'Сервер вернул неизвестный код';
			}
		}).catch(пПричина => {
			if (typeof пПричина == 'string') {
